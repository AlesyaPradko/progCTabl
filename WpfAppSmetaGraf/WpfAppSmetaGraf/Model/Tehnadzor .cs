    ...
    public class Tehnadzor : Worker
    {
        ...
        protected override void ProcessSmeta(List<Excel.Workbook> listAktKStoOneSmeta, Excel.Workbook copySmeta, RangeFile processingArea, string adresSmeta, int size, ref string _textError)
        {
            ...
            if (keyCellNumberPozSmeta != null && keyCellConstructWorkSmeta != null)
            {
                ...
                _aktKSinOrderSort = SortAktKSforTehnadzor(listAktKStoOneSmeta, ref _textError);
                List<Dictionary<int, double>> forRecordWorkColumnInSmeta = new List<Dictionary<int, double>>();
                List<string> nameAktKSRecordColumn = new List<string>();
                string error = null;
                if (_aktKSinOrderSort.Count != 0)
                {
                    Parallel.For(0, _aktKSinOrderSort.Count, new ParallelOptions { MaxDegreeOfParallelism = 8 }, numKS =>
                    {
                        Dictionary<int, double> totalScopeWorkAktKSone = new Dictionary<int, double>();
                        Excel.Worksheet workSheetAktKS = _aktKSinOrderSort[numKS].Sheets[1];
                        Excel.Range rangeAktKS = workSheetAktKS.get_Range(processingArea.FirstCell, processingArea.LastCell);
                        string nameAktKS = null;
                        WorkWithAktKSTehnadzor(workSheetAktKS, rangeAktKS, _aktKSinOrderSort[numKS].FullName, ref error, ref nameAktKS, ref totalScopeWorkAktKSone);
                        forRecordWorkColumnInSmeta.Add(totalScopeWorkAktKSone);
                        nameAktKSRecordColumn.Add(nameAktKS);
                ...
                FormatRecordCopySmeta(sheetCopySmeta, rangeSmetaOne, size);
                if (_aktKSinOrderSort.Count != 0)
                {
        ...
        private void RecordFileTehnadzor(Excel.Worksheet SheetcopySmetaOne, Excel.Range rangeSmetaOne, List<Dictionary<int, double>> forRecordWorkColumninSmeta, List<string> nameAktKSRecordColumn, ref int nextInsertColumn, ref string _textError, string adresSmeta)
        {
            ...
            for (int u = 0; u < _aktKSinOrderSort.Count; u++)
            {
                ...
                {
                    MatchCollection matchNumAkt = nameAkt.Matches(nameAktKSRecordColumn[i]);
                    if (matchNumAkt.Count > 0)
                    {
                        ...
                        {
                            if (numFind[ii] >= '0' && numFind[ii] <= '9')
                            { numberFind += numFind[ii]; }
                        }
                    ...
                    if (_aktKSinOrderSort[u].FullName.Contains(numberFind))
                    {
                        ...
                        string hhhhh = nameAktKSRecordColumn[i];
