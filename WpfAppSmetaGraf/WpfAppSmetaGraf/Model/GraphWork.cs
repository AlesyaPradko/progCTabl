    ...
    public class GraphWork
    {
            ...
            try
            {
                ...
                if (keyCellNumberPozSmeta != null && keyCellColumnTrudozatrat != null)
                {
                    ...
                    using (AllContext db = new AllContext())
                    {
                        ...
                        {
                            ChapterAll.Add(detail.NameChapter);
                            if (detail.NameWork != null)
                            { forChapterlAll.Add(detail.NameWork); }
                            else continue;
                        }
                    }
                    if (forChapterlAll.Count != 0)
                    {
                        ...
                        {
                            RankingAllWorksInOrder(ChapterAll[i], FORChapterAll[i], ref _allChapterInOrder);
                        }
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < ChapterAll.Count; i++)
                        {
                            RankingAllWorksInOrder(ChapterAll[i], ref _allChapterInOrder);
                        }
                    }
                    Marshal.FinalReleaseComObject(rangeSmeta);
                    Marshal.FinalReleaseComObject(workSheetSmeta);
        ...
        private List<int> GetFirstPosChapter(Excel.Worksheet workSheetoneSmeta, ref string _textError)
        {
            ...
            try
            {
                ...
                {
                    Excel.Range startChapt = workSheetoneSmeta.Cells[_cellsAllChapter[j].Row + 1, _cellsAllChapter[j].Column];
                    ...
                    {
                        startChapt = workSheetoneSmeta.Cells[_cellsAllChapter[j].Row + 2, _cellsAllChapter[j].Column];
                        if (startChapt != null && startChapt.Value2 != null && !startChapt.MergeCells && startChapt.Value2.ToString() != "" && startChapt != null)
                        {
                            startChapter.Add(Convert.ToInt32(startChapt.Value2));
                        }
        ...
        {
            int inputWorkersMax = (int)(0.18 * _trudozatratTotal / 8);
                        ...
                        try
                        {
                            for (int i = 0; i < keyTrudozatratEachWork.Length; i++)
                            {
                                ...
                                if (numPosSmeta == keyTrudozatratEachWork[i])
                                {
                                    ...
                                    for (int k = 0; k < _cellsAllChapter.Count; k++)
                                    {
        ...
        private void RankingAllWorksInOrder(string regulNameOfRazdel, Regex regulNameWorkOfRazdel, ref List<Dictionary<int, int>> _allRazdelInOrder)
        {
            ...
            for (int i = 0; i < _cellsAllChapter.Count; i++)
            {
                string stringPoRazdelyforPoisk = _cellsAllChapter[i].Value.ToString();
                if (stringPoRazdelyforPoisk.Contains(regulNameOfRazdel))
                {
                    if (i < _cellsAllChapter.Count - 1)
                    {
        ...
        public void RecordAllString(int amountOfWorkers, Excel.Worksheet workSheetGrafik, ref int amountOfWorkInChapter, ref int[] numChapterTablExcelGrafik, ref int[] amountOfWorkersInChapter)
        {
            ...
            double[] trudozatratForChapter = _onChapterTrudozatrat.Values.ToArray();
            string[] valueNameOfEachWork = _nameForEachWorkinSmeta.Values.ToArray();
            ...
            for (int i = 0; i < _allChapterInOrder.Count; i++)
            {
                ...
                int daysOfEachWork;
                for (int r = 0; r < _cellsAllChapter.Count; r++)
                {
                    ...
                    else if ((keyNumPosSmetaChapterInOrder[indexAmountOfRowEachWorkinChapter] == _startChapter[r] && _amounWorkInChapter[_startChapter[r]] > keyNumPosSmetaChapterInOrder.Length) || ((r == _cellsAllChapter.Count - 1 || keyNumPosSmetaChapterInOrder[indexAmountOfRowEachWorkinChapter] < _startChapter[r + 1]) && keyNumPosSmetaChapterInOrder[indexAmountOfRowEachWorkinChapter] > _startChapter[r] && _amounWorkInChapter[_startChapter[r]] > keyNumPosSmetaChapterInOrder.Length))
                    {
                        indexAmountOfRowEachWorkinChapter = 0;
                        string nameOfRazdel = _cellsAllChapter[r].Value.ToString();
                        workSheetGrafik.Cells[firstCellAfterContent.Row + amountOfWorkInChapter, firstCellAfterContent.Column] = ++numPosGrafik;
                    ...
                    do
                    {
                        if (r < _cellsAllChapter.Count - 1 && keyNumPosSmetaChapterInOrder[indexAmountWorkInChapter] >= _startChapter[r + 1])
                        {
                            break;
                        }
